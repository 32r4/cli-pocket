...
where
    T: Transport,
{
    let AcceptedTransport { label, transport } = accepted;
    let psk_ref = deps.psk.as_deref();
    let result =
        run_connection_with_handshake(transport, &deps.identity, psk_ref, deps.connection_deps())
            .await;
